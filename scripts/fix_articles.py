#!/usr/bin/env python3
"""
Fix article vocab definitions:
Remove the '；常用來談XX' suffix and provide proper Chinese definitions.
"""
...
# Proper Chinese definitions for all article vocab words
ARTICLE_VOCAB_DEFS = {
    'adapting': '適應；隨情況調整',
    'aging': '老化；年老',
    'animals': '動物',
...
def fix_article_file(filepath):
    ...
    def replace_def(m):
        word = m.group(1)
        old_def = m.group(2)
        if '；常用來談' in old_def and word in ARTICLE_VOCAB_DEFS:
            new_def = ARTICLE_VOCAB_DEFS[word]
            changes.append(f"  '{word}': def fixed")
            return f"word: '{word}', definition: '{new_def}'"
        return m.group(0)
...
def main():
    files = sorted(glob.glob(os.path.join(BASE, 'content/articles/articles-w*.ts')))
    total_files = 0
    total_changes = 0
    ...
    for filepath in files:
        ...
        changed, changes = fix_article_file(filepath)
        if changed:
            total_files += 1
            total_changes += len(changes)
            print(f"✓ {fname}: {len(changes)} defs fixed")
