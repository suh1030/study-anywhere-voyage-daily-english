...
- art:W20D6：en 文中補入 synthesize（turning→synthesizing）、proactively（主動分享），中文語意不變、已對齊 → 修
- art:W48D7：字彙 communal 未在文中，改為文中確實出現且主題核心的 expressive → 修
- art:W12：文章 D4–D7 重排（Traditions/Conflict/Aging/Parenting），使 D1–D6 與 episode 主題對齊；
  **唯一殘留 D7**（Chosen Family 集配到 Parenting Styles 文，因文章組無 Chosen Family 主題文）——
  要完全對齊需把該篇改寫為 Chosen Family 主題（新內容，待拍板）。記於 marks.json。
