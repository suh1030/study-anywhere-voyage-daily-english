...
### 追加審查與修復（2026-07-10 第二輪）：字卡溯源與重複

第二輪針對性檢查發現兩個既有腳本從未涵蓋的問題，均已修復：

1. **字卡溯源失敗 284/583（49%）**：headword 不存在於對應週的來源內容（listen→該週集數+keyPhrases；speak→該週文章+vocabulary）。根因：文章多輪改寫縮短時把來源句刪掉、早期清理（87790bc，89 筆）標準過鬆。
2. **跨週重複 headword 13 組**（sleep debt、set the tone、burnout 等各出現 2 次）。

**修復方式**：共替換 **287 張卡**（284 脫鉤 + 3 張重複組中的後週卡）。替換來源為該週真實內容中已審修過的學習單位——listen 卡自集數 keyPhrases 提拔、speak 卡自文章 vocabulary 提拔（沿用其中英釋義與例句），經人工逐卡覆核（約 90 張改選以排除學術詞/發明詞）、49 個例句加長至 ≥12 字、3 個中文釋義去英文化。驗證結果：**溯源失敗 0、重複 headword 0、重複例句 0、重複 id 0**，全套 prelaunch 檢查與 tsc 綠燈。

**流程缺口已補**：溯源與 headword 全域唯一檢查已正式納入 `scripts/validate-supporting-content.js`（含負向測試驗證），未來任何內容改寫若再造成字卡脫鉤或重複，validator 會直接紅燈。

### P1 — ✅ 已於 2026-07-10 第三輪全部完成

- [x] **口頭禪稀釋（766 行，中英同步）**："I can see that."×312、"Mostly, yes."×116、"Exactly right."×97、"I can see why you'd say that."×61 以多變體輪替稀釋（填充詞類 zh 本就未翻譯、僅動 en；成對類 en+zh 同步輪替）；11 個鑄造感整句反應（~95 行）換為 28 組自然反應句池，各留 2 次原句；13 個 opener 口頭禪成對輪替。稀釋後全庫最高頻殘留為自然附和「That's true.」×94（真人口語本就重複，判定保留）。教學段固定開場（"Here is the version I would actually use." 等）判定為節目格式語言，刻意保留。
  - 修正審查誤判：原 F1 疑似的「I can see that. 類 312 行 en/zh 脫鉤」經查證為**填充詞慣例性省譯**，非脫鉤。
- [x] **W30 類抽象對白改寫**（interior motion 段落等，中英同步）+ 殘餘 "That maps well onto the actual experience."×4 分散 + F7 文法錯（feet are/were、could…fits）。
- [x] **C1 文章降級（16+2 篇、188 處改寫）**：以難度掃描（學術詞密度+長詞率+句長）找出離群者，逐句拆短改寫至 B2（平均句長 26–36 → ≤22），語意不變故中文翻譯維持有效；W31 awe 文章難句軟化（去人名堆疊、去 "downstream of"）。掃描分數 max 91→67、p90 50→44，殘餘高分皆為合理 B2 領域詞。改寫後 wordCount 已重校（0 outliers）、字卡溯源重驗 0 失敗。
- [x] **SKILL.md 全面重寫（v2.0.0）**：對齊 53 週×365 現實結構、文章跟週主題、行內 vocab 退役、字卡溯源+全域唯一規則、口頭禪紅線、難度紅線、務實的地名/品牌政策、必跑驗證清單與四條歷史教訓。
